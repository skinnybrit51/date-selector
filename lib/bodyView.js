var moment = require('moment'),
    tableCellTemplate = require('./templates/tableCellTemplate.hbs');
...
module.exports = function (selectedMonth, selectedDate, options) {
    ...
    var startingDay = options.startingDay,
    ...
    var days = [];
    if (startDay >= startingDay) {
        // calculate how many days to display from previous month
        for (i = (daysInPreviousMonth - (startDay - startingDay)) + 1; i <= daysInPreviousMonth; i++) {
            days.push(previousMoment.clone().date(i));
        }
    } else {
        var diff = startingDay - startDay;
        for (i = (daysInPreviousMonth - (7 - diff)) + 1; i <= daysInPreviousMonth; i++) {
            days.push(previousMoment.clone().date(i));
        }
    }
    ...
    for (var w = 1; w <= 6; w++) {
        ...
        for (var d = 1; d <= 7; d++) {
            ...
            if (selectedDateNow && (aNow === selectedDateNow)) {
                ...
                title = lang.selected_day;
            }

            markup += tableCellTemplate({
                title : title,
                classes : classes.join(' '),
                date : aMoment.format('YYYY-MM-DD'),
                monthDay : aMoment.date()
            });
